...
const PLANT_DB: Record<string, any> = {
  tomato: {
    ...
    color: "text-red-500",
    bg: "bg-red-500",
    specs: { temp: "20-27°C", air_humid: "60-80%", soil_humid: "60-70%", light: "40k-60k lx", water: "2-3cm/week" }
  },
  lettuce: {
    name: "Lettuce",
    image: "/lettuce.png",
    ...
    bg: "bg-green-500",
    specs: { temp: "15-20°C", air_humid: "50-70%", soil_humid: "70-80%", light: "12k-20k lx", water: "Keep moist" }
  },
  basil: {
    name: "Basil",
    image: "/basil.png",
    ...
    bg: "bg-emerald-600",
    specs: { temp: "21-29°C", air_humid: "40-60%", soil_humid: "40-60%", light: "30k-50k lx", water: "Moderate" }
  }
};
...
type SensorData = {
  temperature?: string;
  humidity_air?: string;
  humidity_soil?: string;
  light_lux?: string;
  water_level?: string;
  realtime_image_url?: string;
...
const MetricCard = ({ title, value, unit, icon: Icon, colorClass = "text-slate-900 dark:text-white" }: any) => (
  <div className="group relative overflow-hidden bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl p-5 hover:shadow-lg hover:border-emerald-500/30 transition-all">
    <div className="flex justify-between items-start">
      ...
      <div className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-600 dark:text-slate-300 group-hover:text-emerald-500 group-hover:bg-emerald-50 transition-colors">
        <Icon size={22} strokeWidth={2} />
      </div>
    </div>
    <div className="absolute -right-6 -bottom-6 w-24 h-24 bg-gradient-to-br from-slate-200 to-transparent dark:from-slate-800 rounded-full opacity-20 group-hover:scale-150 transition-transform duration-500" />
  </div>
);

// 2. Advanced Camera Feed (Handles MJPEG Stream vs Static Image)
const CameraFeed = ({ url, title, timestamp, isLiveStream = false }: { url?: string; title: string; timestamp?: string; isLiveStream?: boolean }) => {
  // Logic: If it's a static image (daily), add timestamp to break cache. 
  // If it's a live stream (flask), use URL directly to avoid breaking the stream connection.
  const displayUrl = isLiveStream ? url : (url ? `${url}?t=${new Date().getTime()}` : "");
    ...
    <div className="flex flex-col h-full bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
      <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white/50 dark:bg-slate-900/50">
        <div className="flex items-center gap-2 text-slate-700 dark:text-slate-200 font-medium text-sm">
          <div className={`w-2 h-2 rounded-full ${isLiveStream ? "bg-red-500 animate-pulse" : "bg-emerald-500"}`} />
          {title}
        </div>
      ...
      <div className="relative flex-1 min-h-[250px] bg-slate-100 dark:bg-slate-950 flex items-center justify-center group overflow-hidden">
        {url ? (
          <img 
            ...
            // MJPEG streams don't need lazy loading, they need immediate connection
            loading={isLiveStream ? "eager" : "lazy"} 
          />
        ) : (
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
          <button className="flex items-center gap-2 text-xs bg-white/20 hover:bg-white/30 backdrop-blur-md text-white px-3 py-1 rounded-full border border-white/30 transition-colors">
...
const getSeverityColor = (alert: string) => {
  ...
  // Disease detection logic based on your Python classes
  if (text.includes("spot") || text.includes("blight") || text.includes("mold") || text.includes("critical")) {
...
export default function IoTDashboard() {
  ...
  const { data, error, isLoading, mutate } = useSWR<DashboardResponse>("/api/dashboard", fetcher, {
    refreshInterval: 5000, // Poll faster (5s) to catch alerts quickly
  ...
  const parseAlerts = (raw: any): string[] => {
    if (!raw) return [];
  ...
  const alerts = parseAlerts(sensor.alerts);
  const isOnline = !error && data?.sensor_data;

  // Auto-detect if the URL is an MJPEG stream (usually port 5000 or ends in /video_feed)
  const isStream = sensor.realtime_image_url?.includes(":5000") || sensor.realtime_image_url?.includes("video_feed");
      ...
      <nav className="sticky top-0 z-30 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => mutate()} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors text-slate-400 hover:text-slate-600">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 flex items-center gap-2 text-sm">
              <Clock size={14} />
              Last update: {data?.timestamp ? new Date(data.timestamp).toLocaleString() : "Syncing..."}
            </p>
          </div>
          ...
          <div className="relative group min-w-[200px]">
            ...
            <div className="relative">
              ...
              >
                {Object.keys(PLANT_DB).map((key) => (
                  <option key={key} value={key}>{PLANT_DB[key].name}</option>
                ))}
              </select>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="w-full sm:w-[30%] bg-slate-100 dark:bg-slate-800 relative flex items-center justify-center p-4">
                 ...
                 <div className="relative z-10 flex flex-col items-center text-center">
                    <div className="w-24 h-24 relative mb-3">
                      ...
                      />
                      <div className="hidden w-full h-full flex items-center justify-center text-emerald-500">
                        <Leaf size={64} />
                      </div>
                    </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="grid grid-cols-1 gap-4 h-[500px]">
               <CameraFeed 
                 title="Live Stream (Pi)" 
                 url={sensor.realtime_image_url} // URL from Sheet (e.g., http://192.168.1.45:5000/video_feed)
                 timestamp={sensor.timestamp}
                 isLiveStream={isStream} // Use simple logic to detect stream
               />
               <CameraFeed 
                 title="Daily Snapshot (6PM)" 
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  {weather?.weather[0]?.icon && (
                    <img 
                      ...
                      alt="weather icon" 
                      className="w-16 h-16 -mt-2 filter drop-shadow-md"
                    />
                  )}
                </div>
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-sm text-blue-50">
                    <span className="flex items-center gap-2"><Droplets size={14}/> Humidity</span>
                    <span>{weather?.main?.humidity ?? "--"}%</span>
                  </div>
                  <div className="flex justify-between items-center text-sm text-blue-50">
                    <span className="flex items-center gap-2"><Activity size={14}/> Wind</span>
                    <span>{weather?.wind?.speed ?? "--"} m/s</span>
                  </div>
                  <div className="mt-4 pt-4 border-t border-white/20 text-xs text-blue-100 text-center capitalize">
                    {weather?.weather[0]?.description ?? "Loading weather data..."}
                  </div>
                </div>
              </div>
            </div>
